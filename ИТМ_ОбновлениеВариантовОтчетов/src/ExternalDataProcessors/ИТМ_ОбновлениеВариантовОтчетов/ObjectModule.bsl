...
// 
// Отключение проверок:
// sonar:MethodCognitiveComplexity
// 
// Параметры:
...
//
Процедура ОбработатьНастройкиОтчета(Данные, Знач ПараметрыВыполнения = Неопределено) Экспорт // sonar:MethodCognitiveComplexity
	...
	Если ЭтоТипНастройкиКомпоновкиДанныхИПодобныеИм(ТипДанных) Тогда
		...
		ОбработатьНастройкиОтчета_НастройкиКомпоновкиДанныхИПодобныеИм(Данные, ПараметрыВыполнения);
		
	ИначеЕсли ЭтоТипСЭлементамиКомпоновки(ТипДанных) Тогда
		
		ОбработатьНастройкиОтчета_ТипСЭлементами(Данные, ПараметрыВыполнения);
		
	ИначеЕсли ЭтоТипКоллекцияЭлементовКомпоновки(ТипДанных) Тогда
		
		ОбработатьНастройкиОтчета_КоллекцияЭлементов(Данные, ПараметрыВыполнения);
		
	ИначеЕсли ТипДанных = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
		
		ОбработатьНастройкиОтчета(Данные.Настройки, ПараметрыВыполнения);
		
	ИначеЕсли ТипДанных = Тип("ПользовательскоеПолеВыборКомпоновкиДанных") Тогда
		
		ОбработатьНастройкиОтчета(Данные.Варианты, ПараметрыВыполнения);
		
	ИначеЕсли ТипДанных = Тип("ВариантПользовательскогоПоляВыборКомпоновкиДанных") Тогда
		
		ОбработатьНастройкиОтчета_ВариантПользовательскогоПоля(Данные, ПроверяемыеСвойства, ПараметрыВыполнения);
	...
	ИначеЕсли ТипДанных = Тип("ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
		...
		ОбработатьНастройкиОтчета_ЭлементУсловногоОформления(Данные, ПараметрыВыполнения);
	...
	ИначеЕсли ЭтоТипЗначениеПараметра(ТипДанных) Тогда
		...
		ОбработатьНастройкиОтчета_ЗначениеПараметра(Данные, ПроверяемыеСвойства, ПараметрыВыполнения);
...
Процедура ОбработатьНастройкиОтчета_НастройкиКомпоновкиДанныхИПодобныеИм(Данные, ПараметрыВыполнения)
	...
	Для Каждого КлючИЗначение Из ТребуемыеСвойства Цикл
		...
		Если ТекущаяКоллекция <> Неопределено Тогда
			ОбработатьНастройкиОтчета(ТекущаяКоллекция, ПараметрыВыполнения);
		КонецЕсли;							
	КонецЦикла;
	
КонецПроцедуры

Процедура ОбработатьНастройкиОтчета_ТипСЭлементами(Данные, ПараметрыВыполнения)
	
	Для Каждого Элемент Из Данные.Элементы Цикл
		ОбработатьНастройкиОтчета(Элемент, ПараметрыВыполнения);
	КонецЦикла;
	
КонецПроцедуры

Процедура ОбработатьНастройкиОтчета_КоллекцияЭлементов(Данные, ПараметрыВыполнения)
...
Процедура ОбработатьНастройкиОтчета_ЗначениеПараметра(Данные, ПроверяемыеСвойства, ПараметрыВыполнения)
	
	ПроверяемыеСвойства.Добавить("Значение");
	ОбработатьНастройкиОтчета(Данные.ЗначенияВложенныхПараметров, ПараметрыВыполнения);
	
КонецПроцедуры

Процедура ОбработатьНастройкиОтчета_ЭлементУсловногоОформления(Данные, ПараметрыВыполнения)
	
	ОбработатьНастройкиОтчета(Данные.Отбор, ПараметрыВыполнения);
	ОбработатьНастройкиОтчета(Данные.Поля, ПараметрыВыполнения);
	ОбработатьНастройкиОтчета(Данные.Оформление, ПараметрыВыполнения);
	
КонецПроцедуры

Процедура ОбработатьНастройкиОтчета_ВариантПользовательскогоПоля(Данные, ПроверяемыеСвойства, ПараметрыВыполнения)
