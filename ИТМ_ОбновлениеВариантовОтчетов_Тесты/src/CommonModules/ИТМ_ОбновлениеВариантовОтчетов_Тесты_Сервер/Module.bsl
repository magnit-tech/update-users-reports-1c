...
Процедура ИсполняемыеСценарии() Экспорт
    ...
    ЮТТесты
    	...
    	.ДобавитьТестовыйНабор("ЗаполнитьОбновитьВариантыОтчетов")
    		.ВТранзакции().Перед("ПередНабором_ЗаполнитьОбновитьВариантыОтчетов")
    			.ДобавитьТест("Тест_ЗаполнитьВариантыОтчетов")
    			.ДобавитьТест("Тест_ОбновитьВариантыОтчетов")
    	;
...
// Контекст набора заполнить варианты отчетов.
// 
// Возвращаемое значение:
//  Структура - :
...
Функция КонтекстНабора_ЗаполнитьОбновитьВариантыОтчетов() Экспорт
	
	Перем ЮТест;
	ЮТест = ЮТест();
	
	ДанныеКонтекста = Новый Структура(); //см. КонтекстНабора_ЗаполнитьОбновитьВариантыОтчетов
	...
	ПараметрыЗаписи = Новый Структура("ОбменДаннымиЗагрузка", Истина);
	
	ТестовыйОтчет = ЮТест.Данные().СоздатьЭлемент(
		Метаданные.Справочники.ИдентификаторыОбъектовМетаданных, 
		МетаданныеТестовогоОтчета.Имя, 
		Новый Структура("ПолноеИмя", МетаданныеТестовогоОтчета.ПолноеИмя()),
		ПараметрыЗаписи); //СправочникСсылка.ИдентификаторыОбъектовМетаданных
	...
	ТестовыйВариант = ЮТест.Данные().СоздатьЭлемент(
		...
		ПараметрыЗаписи); //СправочникСсылка.ВариантыОтчетов
...
Процедура ПередНабором_ЗаполнитьОбновитьВариантыОтчетов() Экспорт
	
	КонтекстНабора_ЗаполнитьОбновитьВариантыОтчетов();
...
Процедура Тест_ЗаполнитьВариантыОтчетов() Экспорт
	...
	Контекст = КонтекстНабора_ЗаполнитьОбновитьВариантыОтчетов();
	
	Обработка = НоваяОбработка();
	Обработка.Инициализировать(Контекст.Отчет);
	Обработка.ЗаполнитьВариантыОтчетов();
	
	ЮТест
		.ОжидаетЧто(Обработка.ОбновляемыеВарианты)
			.ИмеетДлину(1)
			.Свойство("[0].ВариантОтчета").Равно(Контекст.Вариант)
	;
...
Процедура Тест_ОбновитьВариантыОтчетов() Экспорт
	...
	Обработка.ЗаменяемыеПоля.Загрузить(
		Контекст.Обработка.ЗаменяемыеПоля.Выгрузить());
	
	Обработка.ЗаполнитьВариантыОтчетов();
	Обработка.ОбновитьВариантыОтчетов();
	
	СтрокаНастройки = СтрокаНастроек(Контекст.Вариант.Настройки);
	СтрокаНастройкиЭталон = СтрокаНастроек(Контекст.НастройкиПосле);
